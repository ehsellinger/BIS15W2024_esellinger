...
```{r}
summary(msleep24) #It worked! Checked by viewing dataframe
```
...
You can also combine functions to make useful summaries for multiple variables. ## Know for exam!!
```{r}
msleep %>% 
  filter(bodywt > 200) %>% 
  summarize(mean_sleep_large = mean(sleep_total), 
...
msleep %>% 
  filter(order == "Primates") %>% 
   summarize(mean_primates = mean(bodywt),
            min_primates = min(bodywt),
            max_primates = max(bodywt),
            total = n())
```

`n_distinct()` is a very handy way of cleanly presenting the number of distinct observations. Here we show the number of distinct genera over 100 in body weight.  ## on exam!!!
...
```{r}
msleep %>% 
  summarize(n_genera = n_distinct(genus)) # this is going to count the number of genera in msleeps
```
...
```{r}
msleep %>% 
  summarize(mean_mammals = mean(sleep_total),
            min_mammals = min(sleep_total),
            max_mammals = max(sleep_total),
            total = n())
```
...
msleep %>% 
  ...
  select("order", "vore", "brainwt") # some values not recorded
```

```{r}
msleep %>% 
  filter(order == "Cetacea") %>% 
  select("order", "vore", "brainwt") # all value not recorded
```
