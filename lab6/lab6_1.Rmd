...
```{r}
fish <- readr::read_csv("data/Gaeta_etal_CLC_data.csv") #read:: = use the readr package
```
...
```{r}
fish %>% #load fish data
  mutate(length_mm = length*10) %>% #multiply length data by 10 and rename it 'length_mm
  select(fish_id, length, length_mm) #create new dataframe to see new column added
```
...
```{r}
fish %>% #load fish data
  mutate(length_half = length/2) %>% #divide length data by 2 and rename it 'length_half
  select(fish_id, length, length_half) #create new dataframe to see new column added
```
...
mammals %>% 
  select(genus, species, newborn) %>%
  mutate(newborn_new = ifelse(newborn == -999.00, NA, newborn))%>% #created new variable/column "newborn_new, if it is -999, will replace with NA. If not -999, will leave it alone
  arrange(newborn)
```

## Practice
1. We are interested in the family, genus, species and max life variables. Because the max life span for several mammals is unknown, the authors have use -999 in place of NA. Replace all of these values with NA in a new column titled `max_life_new`. Finally, sort the data in descending order by max_life_new. Which mammal has the oldest known life span?
```{r}
mammals %>% 
...
```
#### Balaenoptera physalus has the oldest known life span (the Fin whale)
