...
- hw7: using aminota and amphibio data to find NAs and transform them. 

**Lab 9:**  
- intro: going over final project questions. 
- lab 9_1: Explain the difference between tidy and messy data. Evaluate a data set as tidy or messy. Use the `pivot_longer()` function of `tidyr` to transform data from wide to long format. Use `separate()` to split observations within a column.  
- lab 9_2: Explain the difference between tidy and messy data. Evaluate a data set as tidy or untidy. Use the `pivot_wider()` function of `tidyr` to transform data from long to wide format.  
- hw8: practice transforming between long and wide format data. 

**Lab 10:**  
- intro: long format review. 
- lab 10_1: Understand and apply the syntax of building plots using `ggplot2`. Build a boxplot using `ggplot2`. Build a scatterplot using `ggplot2`. Build a barplot using `ggplot2` and show the difference between `stat=count` and `stat=identity`.  
- lab 10_2: Produce boxplots using `ggplot.` Customize labels on axes using `labs` and `themes`. Use `color`, `fill`, and `group` to customize plots and improve overall aesthetics  
- hw9: plotting practice with colleges stats data. 

**Lab 11:**  
- intro: boxplot practice. 
- lab 11_1: Produce box plots using `ggplot.` Customize labels on axes using `labs` and `themes`. Use `color`, `fill`, and `group` to customize plots and improve aesthetics.  
- lab 11_2: Build stacked bar plots of categorical variables. Build side-by-side barplots using `position= "dodge"`.  
- hw 10: plotting practice with desert species data (labels and colors). 

**Lab 12:**  
- intro: `mutate` and `geom_boxplot` practice with heartrate data. 
- lab 12_1: Build histograms and density plots. Adjust colors using R's built-in color options. Create new categories with `case_when()` and use those categories to build plots. Build line plots.  
- lab 12_2: Adjust the aesthetics of plots in ggplot using `themes`, `RColorBrewer`, and `paletteer.` Use faceting to produce multi-panel plots.   
- hw 11: Tons of plotting practice with gapminder data (great plot examples!)  

**Lab 13:**  
- intro: midterm review and group work  
- lab 13_1: List and join multiple files from a directory.Format dates in a data frame. 
- lab 13_2: 
...
# Summarize NAs: data %>% map_df(~ sum(is.na(.))) OR data %>% naniar::miss_var_summary()
# TOP mins and maxes in data: data %>% slice_max(column, n=5) or data %>% top_n(5, column)
# joining syntax: join_type(firstTable, secondTable, by=columnTojoinOn)
```

**Plotting Quick Code:**
```{r}
# opacity of color: alpha = 0-1 (use in aes)
# remove legends:  guides(fill = FALSE)
# cancels the use of scientific notation for the session options(scipen=999)
# histogram editing: geom_histogram(color = "black", fill = "thistle4", bins=10)
# density editing: geom_density(fill="deepskyblue4", alpha  =0.4, color = "black")
# layering histogram & density: geom_histogram(aes(y = after_stat(density))) + geom_density()
# create new bins using case_when(): data %>% mutate(mass_category = case_when(log10.mass <= 1.75 ~ "small", > 1.75 & log10.mass <= 2.75 ~ "medium", log10.mass > 2.275 ~ "large"))
# have data side by side on bar plot: geom_bar(position = "dodge")
# see lab 12_1 for quartile cutting using 'gtools'
# edit axis tilt: theme(axis.text.x = element_text(angle = 45, hjust=1))
# make y axis on a log scale: scale_y_log10()
# make plots more clear when knitting: ```{r dpi = 250, out.width = "75%"}
```
