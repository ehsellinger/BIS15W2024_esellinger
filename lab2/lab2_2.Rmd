...
```{r}
my_vector_sequence[15] # only shows 15th position, doesn't actually evaluate
...
box_office # combines objects, but they are no longer associated with their titles
```
...
harry_potter_matrix # stacks vectors
```
...
And even add a new column to reflect this calculation. `cbind()` adds columns. Short for column bind. To add row, '`bind()`, short for row bind, creates new row combining all previous rows and uses previous calculation, in this example, `global`.
```{r}
...
```{r}
Totals <- colSums(harry_potter_matrix)
Totals
```
...
```{r}
all_totals_harry_potter_matrix <- rbind(harry_potter_matrix, Totals)
all_totals_harry_potter_matrix
```

## A few Extras
The same methods of selecting elements in a vector apply to data matrices. We use `[]`. The following selects the value in the second row, first column. 
```{r}
harry_potter_matrix[2,1]
```

Adding a colon `:` selects the specified elements in a column. First column, first 4 values.
```{r}
