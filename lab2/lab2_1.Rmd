...
# Use the gets symbol <-, do not use =, will cause problems in some code
```{r}
y <- 30
```
...
3. Use the function `mean()` to calculate the mean top speed for the cars. Multiple ways to do this. Use combine c() function!
```{r}
...
We interpret this result as the inability of R to calculate the mean of the data set due to the presence of an NA, or a missing value.
...
```
This is useful because the function na.rm=T within the mean() function says that it will remove any values that are NA and allows R to calculate the mean.
