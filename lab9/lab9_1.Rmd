...
```{r}
# The days should be values in a column, not each an individual column
```

3. Use `pivot_longer()` to make the data tidy. Focus the data only on genotype, water_sched_prog, and greenhouse.  
```{r}
plant_data %>% 
  pivot_longer(-c(genotype, water_sched_prog, greenhouse), # don't move the columns specified
