...
#### So far this quarter we have practiced many elements of data science as described above. We have learned how to clean up the names of columns from dataframes, select certain columns of interest, filter out data we either do or don't need, and sort the data from increasing to decreasing values (and visversa) for easy viewing. In terms of extracting knowledge, we have learned how to summarize data into organized viewing tables to obtain the mean, min, and max values from certain values within a column of a dataframe.  


```r
# examples of data organzing: clean_names(data), select(data, column1, column2...), filter(data, column == "value"), arrange(data$column)
# examples of extracting knowledge: summarize(data, name == mean(column))
```

2. (2 points) What is the most helpful or interesting thing you have learned so far in BIS 15L? What is something that you think needs more work or practice?  

#### The most helpful thing I have learned so far is the `filter()` function and `group_by()` function. They have really helped me break down the data in meaningful ways. I think I need more practice with the  `across()` function and all its uses.
...
```
## # A tibble: 2 × 5
##   sex   average_height min_height max_height total_elephants
##   <fct>          <dbl>      <dbl>      <dbl>           <int>
## 1 F               232.       193.       278.              37
## 2 M               270.       229.       304.              13
```
...
vertebrates %>% 
  ...
  summarize(birds_diversity = mean(diversity_bird_species),
            mammals_diversity = mean(diversity_mammal_species))
```

```
## # A tibble: 1 × 2
##   birds_diversity mammals_diversity
##             <dbl>             <dbl>
## 1            1.64              1.71
```
...
## 1    2.22     57.0       0.581       32.6      3.22        4.38
```
...
vertebrates %>% 
  ...
  summarize(average_canopy = mean(veg_canopy))
```

```
## # A tibble: 2 × 2
##   hunt_cat average_canopy
##   <fct>             <dbl>
## 1 Moderate           3.78
## 2 None               3.57
```
