...
## Removes scientific notation, always use for ggplot
```{r}
...
```
## Color by continuous variable homerange
```{r}
p + 
  geom_point(size= 1.5, aes(color = log10.hra)) +
    scale_color_viridis()
```
...
penguins %>% 
  ...
  geom_bar() +
  labs(title = "Penguin Counts by Island",
       x = "Island",
       y = "Count",
       fill = "Species")
```
