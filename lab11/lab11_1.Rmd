...
life_history %>% 
  group_by(order) %>%
  summarise(mean_mass = mean(mass, na.rm = T)) %>% 
  ggplot(aes(x = reorder(order, mean_mass), y = mean_mass)) + # order the axis by mean_mass
  geom_col() +
  coord_flip() +
  scale_y_log10()
```
...
elephants %>% 
  ...
  geom_point() +
  geom_smooth(method = lm, se = F) # adds a smoothing model (linear model) and does not plot standard error
```
...
elephants %>% 
  ...
  labs(title="Elephant Age vs. Height", # Adds a title
       x="Age (years)", # Label x axis age
       y="Height (cm)") # Label y axis height
```
...
elephants %>% 
  ...
  labs(title="Elephant Age vs. Height", # Adds a title
       x="Age (years)", # Label x axis age
       y="Height (cm)") + # Label y axis height
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5)) # Says font size is 1.5 bigger than x and y and centers text
```
