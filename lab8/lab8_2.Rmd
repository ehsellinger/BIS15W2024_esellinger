...
```{r}
life_history <- read_csv("data/mammal_lifehistories_v3.csv", na = c("NA", " ", "-999", "-", ".", "not measured")) %>% # need to know how NAs are represented in data. Don't do this by default. Check always!
  clean_names()
```
...
```{r}
naniar::miss_var_summary(life_history) #shows number of NAs and percentage of each variable
```
...
life_history %>% 
  replace_with_na(replace = list(newborn = "not measured", 
                                 ...
                                 mass= -999)) %>% 
miss_var_summary()
# makes replacement of data specific to the variable
```
