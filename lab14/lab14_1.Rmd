...
```{r, eval=F, echo=T}
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm") # making dropdown menu. x dropdown gets the name "Select X Variable" with the listed variables beginning with bill length
)
...
2.  Build a shiny app that produces a barplot that explores `locomotion` and `realm` type with a fill reaction for the variables `trophic.guild` and `thermoregulation`.
...
ui <- fluidPage(
    ...
    selectInput("x", "Select X Variable", choices = c("locomotion", "realm"), 
              selected = "locomotion"),
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
      ...
      homerange %>% 
      ggplot(aes_string(x = input$x, fill = input$fill)) +
      geom_bar(position = "dodge")
